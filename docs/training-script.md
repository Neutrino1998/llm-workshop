...
大家经常听到 ChatGPT、RAG、Agent 这些词。但有一个问题：大部分介绍都是在讲"这个东西能做什么"，很少有人说清楚"它到底是怎么运作的"。今天我们换个角度——我们会先接触一个核心原理，然后通过 6 个递进的演示来感受：所有看起来很复杂的技术，本质上都在做同一件事。

看完之后，大家再看到任何大模型产品，应该都能猜到背后大概是怎么实现的。

## 核心原理（3 分钟）

在进入演示之前，我们先聊今天最重要的一个认知。理解了这一点，后面所有东西都是它的推论。

**大模型的本质就是文字补全。**给它一段文字，它预测下一个字，然后把预测出来的字加到末尾，再预测下一个字，如此循环。就像手机键盘的自动补全——只不过它的补全能力远远超过手机输入法。比如给它"中国的首都是"，它大概率会补出"北京"；或者给它一段英文加"翻译成中文"，它会补出中文翻译。它不"理解"什么，不"思考"什么——它只是在做统计意义上最合理的文字补全。只不过当模型足够大时，这种补全涌现出了令人惊讶的智能行为。
...
这里有一句话值得记住：大模型是无状态的。每次调用都是独立的，它不记得上一次你问了什么。没有 Context 就没有记忆——这不是 bug，这是本质。
...
大家看，第一轮我输入"我叫小明"，messages 里有 system 加 user 加 assistant 三条消息。第二轮问"我叫什么"，messages 变成了五条——整个对话历史都在 Context 里，模型读到"我叫小明"自然能回答。继续聊几轮，token 消耗一直在涨，因为每次都带着完整历史。现在点击"清空"再问"我叫什么"——模型答不上来了，因为 Context 里没有这个信息了。

所以我们要清楚：多轮对话不等于模型记住了，多轮对话等于每次把历史全部传进去。对话越长 Context 越大，成本越高——这就是为什么实际工程中要做上下文管理。

到目前为止，模型只能"说"。下一步，我们让它能"做"——调用外部工具。

## Stage 4：工具调用 —— 在 Context 里加"能力描述"（8 分钟）

模型的知识截止在训练时间点，它无法获取实时信息，也不能查数据库、发邮件。工具调用就是让模型能"做事"的机制。我们先理清完整链路，再看演示。

首先要搞清楚一个问题：**工具在哪里？**工具就是我们开发者自己写的普通函数或者 API——比如调搜索引擎、查数据库、发邮件——它跑在我们自己的服务器上，和模型没有任何关系。模型不知道这些函数存在，除非我们告诉它。

**【演示：在页面顶部"工具试用区"输入"上海今天天气怎么样？"，点"调用 web_search()"】** 大家看，这次调用直接打到后端的搜索 API，返回了一段格式化的文本结果——全程没有大模型参与，就是一次普通的 HTTP 调用。这就是我们系统已经准备好的工具，后面要做的，是让模型也能"用上"它。

怎么让模型用上？我们调用 API 时有一个 `tools` 参数，把工具的定义用 JSON 格式传过去——工具叫什么名字、做什么用、需要哪些参数。这里有个关键细节：**API 提供商拿到这个 tools 参数后，其实就是把工具定义拼进了 prompt。**模型看到的就是一段文字描述，类似"你有以下工具可以使用：web_search，用于搜索互联网……"。这和我们手动在 prompt 里写"你有以下工具可用"是一回事，只不过 API 帮我们标准化和自动化了这个过程。

模型看到这些工具描述后，如果它认为需要使用某个工具，就会补全出一段结构化的 JSON——不是普通的回答文字，而是一个 tool_calls 结构，告诉我们它想调哪个工具、传什么参数。然后我们的代码拿到这个结构，去真正执行对应的函数，把执行结果塞回 Context，再调一次模型。模型看到执行结果，就能基于真实数据补全最终回答了。

关键在于：**模型不执行工具，它只是补全出了"我想调这个工具"的文字。真正执行的是我们的代码。**

所以完整链路是：开发者定义工具 → 通过 API 的 tools 参数传给模型 → API 提供商把工具描述注入 prompt → 模型决策输出 tool_calls → 我们的代码执行工具 → 结果回传模型 → 生成最终回答。
...
第四步是检索。用户的问题也被转成向量，然后和每个 chunk 向量计算相似度，按分数排序返回最相关的几个。大家看到的 0.7 几这个分数，是余弦相似度（cosine similarity）——简单说就是比较两个向量的方向有多接近，值从 0 到 1，越接近 1 说明语义越相近。0.7 以上通常就说明比较相关了。注意，这一步完全没有调用大模型，只是向量数学计算，非常快也非常便宜。
