...
模型的知识截止在训练时间点，它无法获取实时信息，也不能查数据库、发邮件。工具调用就是让模型能"做事"的机制。我先讲清楚完整链路，再看演示。

首先要搞清楚一个问题：**工具在哪里？**工具就是我们开发者自己写的普通函数或者 API——比如调搜索引擎、查数据库、发邮件——它跑在我们自己的服务器上，和模型没有任何关系。模型不知道这些函数存在，除非我们告诉它。

怎么告诉它？我们调用 API 时有一个 `tools` 参数，把工具的定义用 JSON 格式传过去——工具叫什么名字、做什么用、需要哪些参数。这里有个关键细节：**API 提供商拿到这个 tools 参数后，其实就是把工具定义拼进了 prompt。**模型看到的就是一段文字描述，类似"你有以下工具可以使用：web_search，用于搜索互联网……"。这和你手动在 prompt 里写"你有以下工具可用"是一回事，只不过 API 帮你标准化和自动化了这个过程。
