...
**【输入：搜索最新的 AI Agent 框架，对比它们的优劣势】**
**【点击"运行 Agent"】**

看执行轨迹，每一步都能看到 Agent 的"内心活动"：

**🧠 推理与规划**
Agent 分析问题，制定计划。——这就是第一轮 Think。

**🧠 决策：需要搜索**
Agent 判断当前 Context 里没有相关信息，决定调用搜索工具。

**🔧 调用工具：web_search**
后端真正执行搜索，返回结果。——新的信息被加入 Context。

**🔧 调用工具：知识库检索**
同时查查知识库里有没有相关内容。——如果有，继续往 Context 里加。

**🧠 评估信息充分性**
Agent 回顾当前 Context 里所有的信息，判断够不够回答问题。

**📊 生成最终回答**
信息充分了，基于完整的 Context 生成回答。
